    ...
    <form class="max-w-sm mx-auto" method="POST" action="{{ url('tasks/' . $task->id) }}">
        ...
        <div class="mb-5">
            ...
            <input type="text" name="description" id="description"
                ...
                value="{{ old('description', isset($task) ? $task->description : null) }}">
        </div>
        ...
        <div class="mb-5">
            <label for="status-select"
                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Статус:</label>

            <select name="completed" id="completed"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="Выполнено" {{ $task->completed == 'Выполнено' ? 'selected' : '' }}>Выполнено</option>
                <option value="Не выполнено" {{ $task->completed == 'Не выполнено' ? 'selected' : '' }}>Не выполнено
                </option>
            </select>
        </div>
